...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='''
            ...
            Exploratory points are then generated by picking an offset vector
            on a N-dimensional sphere (see --dimensionality and --radii) and
            applying it to the best point. For each exploratory point (see
            --num-proposals) the command is run to evaluate that point. Each
            newly evaluated point is added back to the set of existing points
            and the optimization repeats until convergence (see
            --stale-threshold and --stale-count).''')
    parser.add_argument(
        '-d', '--dimensionality',
